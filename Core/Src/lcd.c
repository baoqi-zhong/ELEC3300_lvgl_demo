...
void LCD_BackLed_Control(uint8_t enable)
{
	if(enable)
...
{
	*(volatile uint16_t *)(FSMC_Addr_LCD_CMD) = usCmd;
}
...
{
	*(volatile uint16_t *)(FSMC_Addr_LCD_DATA) = usData;
}
...
{
	return *(volatile uint16_t *)(FSMC_Addr_LCD_DATA);	
}
...
void LCD_REG_Config(void)
{
	...
	LCD_Write_Cmd(0x36);	
	LCD_Write_Data( 0x08); // Version 2


	/* display inversion */
	LCD_Write_Cmd(0x20);  // Display Inversion ON
